...
mammals_data <- data(package = "MASS", topic= mammals)
Animals2_data <- data(package = "robustbase", topic= Animals2)

help(mammals) #En la parte inferior izquierda nos explican qué información contiene cada variable, como por ejempo que el peso del cuerpo está en kg y el peso del cerebro en g
help(Animals2)

#Primero haremos de los mamíferos
str(mammals)
dim(mammals) #filas x columnas(observacionesx variables)
head(mammals) #6 primeras observaciones
tail(mammals) #6 últimas observaciones

#Ahora del segundo conjunto de datos
str(Animals2)
dim(Animals2)
head(Animals2)
tail(Animals2)

rownames(mammals) #son nombres de los animales
colnames(mammals) #nombres de las variables
...
commonAnimals #Observando el resultado, casi todos los animales están en ambos conjuntos
...
```{r}
setdiff(Animals2, mammals) #hay 3 animales que están en Animals2 que no están en mammals
...
En Animals2 no hay una característica específica común entre ellos; sin embargo en mammals sí. Los 3 animales que no están en mammals son dinosaurios, por lo que es correcto que no se encuentren en los mamíferos ya que eran reptiles.
...
```{r}
qqPlot(log(mammals$brain), ylab="Peso del cerebro", xlab= "Animales mamíferos")
qqPlot(log(Animals2$brain), ylab="Peso del cerebro", xlab= "Animales")

#Podríamos decir que en ambos estrían dentro de una distribución de datos Gaussiana con un 95% de confianza, o un 90% porque el prmer y último animal esán un poco en el borde(a punto de salirse de lalínea discontinua)
...
3.  La función **library** sin argumentos abre una ventana y muestra las librerías que han sido instaladas.
    ...
    ```{r}
libReturn <- library() #es una lista de 3
head(libReturn$results)
dim(libReturn$results) #Como ya sabemos que es una lista de 3, 214 son las librerías que yo tengo instaladas
...
#Primero el número de registros
#Segundo, número de variables
#Y tercero, tipos de vaiables

dim(cabbages) #60 registros y 4 variables
cabbages_data <- cabbages #Al guardarlo como variable, observamos en el desplegable que cult y date son factores, HeadWt es un número y VitC son enteros

sum(is.na(cabbages)) #No hay valores NA

#Ahora con el paquete Chile
dim(Chile) #2700 registros y 8 variables
Chile_data <- Chile #Al guardarlo como variable, observamos en el desplegable que region,sex,education y vote son factores; population,age e income son enteros y; statusquo , es un número

sum(is.na(Chile)) #Hay 295 valores perdidos


#Hacemos summary
summary(cabbages)
summary(Chile) #Por ejemplo en education hay 11 valores perdidos
...
qqPlot(cabbages$HeadWt, ylab="Anchura", xlab= "Lechugas")
qqPlot(cabbages$VitC, ylab="Cantidad Vitamina C", xlab= "Lechugas")
#Con ambas podríamos afirmar con un 95% que sigue la distribución Gaussiana
...
```{r}
hist(cabbages$VitC[cabbages$Cult == "c39"],xlab="Lechugas c39",main = "Vitamina C en lechugas c39", col= "green")
hist(cabbages$VitC[cabbages$Cult == "c52"],xlab="lechugas c52",main = "Vitamina C en lechugas c52", col= "darkgreen")
...
#Diagrama de dispersión etiquetando según un factor.
plot(cabbages$HeadWt, cabbages$VitC, pch=c(6,16)[cabbages$Cult],xlab= "Anchura", ylab= "Vitamina C")
